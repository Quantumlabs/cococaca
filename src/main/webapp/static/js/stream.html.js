 ...
 * */
/******************************@ Globe variables definition start.******************************/
...
function create_viewers_in_post(post_ID, user_IDs) {
    ...
    for (var user_ID in user_IDs) {
        var div = $("<div></div>");
        div.class = "col-xs-2";
        var avatar_img = $(sformat("<img src=\"{0}\" class=\"img-circle center-block img-responsive\"/>", sformat(_URL_CDN_AVATAR_FORMAT, user_ID)));
...
function build_card_header(post_info) {
    ...
    var post_author_avatar = $(sformat("<a><img class=\"img-circle img-responsive\" src=\"{0}\" /></a>",
        _DEBUG ? "http://placehold.it/65x65&text=avatar" : sformat(_URL_CDN_AVATAR_FORMAT, post_info.author)));
...
function build_card_content_container(post_info) {
    ...
    var inner_card_content_container =
        $("<div name=\"inner-card-content-container\"onclick=\"javascript:card_content_on_click()\"></div>");
    var img = $(sformat("<img class=\"img-responsive center-block\"  src=\"{0}\">",
        _DEBUG ? "http://placehold.it/600x600&text=avatar" : sformat(_URL_CDN_CONTENT_IMG_FORMAT, post_info.content)));
...
function build_danmuku(post_info) {
    ...
    //var card_danmuku_content_container = $(parent_card_content_container).find("div[name=card_danmuku_content_container]");
    for (var idx = 0; idx < post_info.danmuku.length; idx++) {
        ...
        var danmuku_div =
            $(sformat("<div class=\"item\"><div name=\"danmuku-author\" class=\"danmuku-init img-circle\"></div>{0}</div>", danmuku.content));
...
function build_card_foot(post_info) {
    ...
    var card_foot_row_content_description =
        $(sformat("<div class=\"h3\" name=\"content-description\">{0}</div>", post_info.description));
    card_foot_row_content_description.appendTo(card_foot);

    var card_foot_row_show_viewer = $("<div name=\"show-viewer\" class=\"row\"></div>");
    var script = $("<script> load_commented_users(\"{0}\")</script>", post_info.postID)
...
function build_post_item(post_info) {
    var row = $("<div name=\"post-item\"class=\"row\" id=\"" + post_info.postID + "\"></div>");
...
function register_footer_call_back() {
    {
        ...
        for (var idx = 0; idx < li_children.length; idx++) {
            li_children[idx].onclick = function (obj) {
                if (_DEBUG) {
                    log_debug(sformat("Click on footer :{0}", obj.srcElement.innerText));
                }
...
$(document).ready(function () {
    ...
    if (_DEBUG) {
        ...
        for (var idx = 0; idx < 10; idx++) {
            var dummy_post_info = {"postID": idx, "author": "12345", "content": "123123", "description": "This is dummy post description", danmuku: [
                {author: "dummy-danmuku-author", content: "dummy-danmuku-content", timestamp: "dummy-danmuku-timestamp"},
                {author: "dummy-danmuku-author-2", content: "dummy-danmuku-content-2", timestamp: "dummy-danmuku-timestamp-2"}
            ]};
